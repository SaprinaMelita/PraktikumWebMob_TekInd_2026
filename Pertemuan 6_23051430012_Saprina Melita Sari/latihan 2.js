...
// CACAT
document.getElementById("jumlahCacat").innerText = daftarCacat.length;

function renderListCacat() {
    let list = document.getElementById("listCacat");
    list.innerHTML = "";
    daftarCacat.forEach((item, index) => {
        let li = document.createElement("li");
        li.className = "list-group-item";
        li.innerText = "Item ke-" + (index + 1) + " : " + item;
        list.appendChild(li);
    });
}
...
function tambahData() {
    let kode = "C-" + (Math.floor(Math.random() * 900) + 100);
    daftarCacat.push(kode);
    document.getElementById("jumlahCacat").innerText = daftarCacat.length;
    renderListCacat();
}

// MESIN
document.getElementById("namaMesin").innerText =
    "Nama: " + mesinA.nama;

document.getElementById("statusMesin").innerText =
    "Status: " + mesinA.status;

document.getElementById("tekananMesin").innerText =
    "Tekanan Max: " + mesinA.spesifikasi.tekananMax + " bar";

// GUDANG
let gudangList = document.getElementById("gudangList");

gudangMaterial.forEach(item => {
    let li = document.createElement("li");
    li.className = "list-group-item";

    if (item.stok < 20) {
        li.innerText = item.nama + " (" + item.stok + ") ⚠️ Stok menipis!";
        li.style.color = "red";
    } else {
        li.innerText = item.nama + " (" + item.stok + ")";
    }
...
function maintenanceMode() {
    ...
    // Ubah warna alert indicator
    statusIndicator.className = "alert alert-secondary";

    // Ambil card Panel Kontrol secara spesifik lewat statusIndicator
    let cardBody = statusIndicator.closest('.card-body');
...
// LATIHAN 2 - Mouseover Suhu Mesin
